...
core.register_entity("1042_core:campfire_fire", {
	initial_properties = {
		physical = false,
		pointable = false,
...
core_1042.phases.register_callback("complex_registration", function()
	for id, def in pairs(core.registered_items) do
		...
		if thing then
			core.register_entity(":1042_core:campfire_cooking_"  .. thing.name, {
				initial_properties = {
					hp_max = 1000,
					physical = false,
					pointable = false,
...
core.register_node("1042_core:campfire", {
	...
	on_construct = function(pos)
		core.add_entity(pos, "1042_core:campfire_fire", nil)
	end,
	...
	on_destruct = function(pos)
		for object in core.objects_inside_radius(pos, 0.5) do
			...
			if entity then
				if entity.name == "1042_core:campfire_fire" then
	...
	on_rightclick = function(pos, node, player, itemstack, pointed_thing)
		...
		for object in core.objects_inside_radius(pos, 0.5) do
			local entity = object:get_luaentity()
			if entity then
				if string.find(entity.name, "1042_core:campfire_cooking_") then
					local thing = core.registered_items[name]._1042_campfire_cooks

					if thing and "1042_core:campfire_cooking_" .. thing.name == entity.name then
		...
		if thing then
			if (not has_hanging) and thing.hanging then
				itemstack:take_item()
				core.add_entity(pos, "1042_core:campfire_cooking_" .. thing.name, nil)
			elseif (not thing.hanging) and has_side < 4 then
				...
				core.add_entity(moved_pos, "1042_core:campfire_cooking_" .. thing.name, nil)
			end
		end
