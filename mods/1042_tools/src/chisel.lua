...
local function chisel(chisel_data, player, seconds, node_being_chiseled)
    ...
    if seconds == chisel_data.duration then
		...
		core.after(.5, function()
			chisel(chisel_data, player, seconds+.5, node_being_chiseled)
		end)
	end
end
        ...
        for _, chisel_data in ipairs(rec) do
            ...
            if chisel_data.check then
                if chisel_data.check(pos) then
                    ...
                    chisel(chisel_data, user, 0, node_being_chiseled)
                    break
                end
            else
                itemstack = item_wear.wear(itemstack, math.ceil(chisel_data.duration))
